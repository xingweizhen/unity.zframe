...
public static class LibNetwork
{
    ...
    private enum MsgField { PACK, Int, Long, Float, String, }

    private static void UnpackField(ILuaState lua, MsgField field, int index)
    {
        switch (field) {
            case MsgField.Int: lua.PushInteger(readNm.readU32()); break;
            case MsgField.Long: lua.PushLong(readNm.readU32()); break;
            case MsgField.Float: lua.PushNumber(readNm.readFloat()); break;
            case MsgField.String: lua.PushString(readNm.readString()); break;
            default:
