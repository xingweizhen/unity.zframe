...
public static class LibAsset
{
    ...
    {
        var loader = AssetsMgr.Instance.Loader;
        ...
        if (tasks.Count > 0) {
            ...
            AssetsMgr.Instance.Loader.BeginMultitasking(onLoaded, param);
            for (int i = 0; i < tasks.Count; i++) {
                AssetsMgr.Instance.Loader.ScheduleTask(tasks[i]);
            }

            AssetsMgr.Instance.Loader.EndMultitasking();
        } else {
            // 没有需要加载的资源，直接执行回调函数
            if (lua.IsFunction(2)) {
                LuaOnLoaded(lua, 2);
    ...
    private static int Unload(ILuaState lua)
    {
        ...
        if (string.IsNullOrEmpty(asstPath)) {
            AssetsMgr.Instance.Loader.UnloadAll();
        } else {
            AssetsMgr.Instance.Loader.Unload(asstPath, lua.OptBoolean(2, false));
        }
        return 0;
    }
    ...
    private static int StopLoading(ILuaState lua)
    {
        ...
        AssetsMgr.Instance.Loader.StopLoading(assetPath: param, keepPreload: keep);
        return 0;
    }
    ...
    private static int ViewLoading(ILuaState lua)
    {
        ...
        if (func != null) {
            AssetsMgr.Instance.Loader.Loading = (bundle, f) => {
            ...
            AssetsMgr.Instance.Loader.Loading = null;
        }
        return 0;
    }
    ...
    private static int LimitAssetBundle(ILuaState lua)
    {
        ...
        var nUnload = AssetsMgr.Instance.Loader.LimitAssetBundle(group, limit);
        if (nUnload > 0) {
