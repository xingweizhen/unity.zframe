...
namespace TinyJSON
{
	public abstract class Variant : IConvertible
	{
		...
		public static int s_GlobalIndent;
		protected const string INDENT_STEP = "    ";
		protected static void generateIndent(out string curr, out string next)
		{
			System.Text.StringBuilder strbld = new System.Text.StringBuilder();
			for (int i = 0; i < s_GlobalIndent; ++i) {
				strbld.Append(INDENT_STEP);
			}
			curr = strbld.ToString();
			next = strbld.Append(INDENT_STEP).ToString();
		}
		public abstract string ToJSONString(bool prettyPrinted = false);
        ...
        public abstract void Set(IConvertible value);
		...
		public static implicit operator String(  Variant variant ) { return variant != null ? variant.ToString(  formatProvider ) : null; }       
